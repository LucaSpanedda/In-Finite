...
AgentCulture(N, M, S) =  
    (netInsx : delays(S) : gains : netLimt : hgroup("ControlSignals", cntrlSignal) <: 
...
with{
    ...
    cntrlSignal = si.bus(N) <: (si.bus(N), par(i, N, controlSignalProcessing(S, i, 48, 10) : si.smoo));
    // Mapping
    ...
    // Agents
    Agent(0) = hgroup("BP Agent", routeSig : MapBP : par(i, N, BPSVF(1)));
    Agent(1) = hgroup("AP Agent", routeSig : MapAP : par(i, N, AllpassWernerCascade(8, 192000, 60 * ma.SR)));
    Agent(2) = hgroup("GN Agent", routeSig : MapGN : par(i, N, Stretcher(30, 30, 1, 1, 1, 0, 10, i)));  
};
// Populations
